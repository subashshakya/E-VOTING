...
import 'package:evoting_ui/user_view/sub_mayor_voting.dart';
...
class SelectableCandidate extends StatefulWidget {
  ...
  final bool selected;
  final String token;
  final List<CandidateGet> subMayor;
...
class _SelectableCandidateState extends State<SelectableCandidate> {
  ...
  Future sendVote() async {
    ...
    if (confirmation) {
      ...
      //     },
      //     body: jsonEncode(<String, String>{"ciphertext": encryptedVote}));
  ...
  String Vencryption(int id, String nominatedYear) {
    ...
    log(encryptedString);
    setState(() {
      encryptedVotingDetails = encryptedString;
    });
    // log(decrypted);
    // log(encryptedString);
  ...
  void push() {
    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
            builder: (context) => SubMayorVotingView(
                widget.token, widget.subMayor, encryptedVotingDetails)));
  }
